...
import { setOverride } from "./ScheduledTask/Scheduler";

export function OutputToggleBase(
	outputName: string,
	outputDescription: string = "",
	toggleType: ToggleType,
	executeFunction: (event: ToggleEvent) => Promise<ToggleResult>
) {
	...
	SharedEventBus.on("ToggleEvent", (event: ToggleEvent) => {
		if (event.toggleName == outputName) {
			...
			if (tryLockOutput(outputName)) {
				...
				// Set override flag for scheduler.
				setOverride(outputName, event.toggleValue);
	...
	SharedEventBus.on("ToggleEventSystemTriggered", (event: ToggleEvent) => {
		if (event.toggleName == outputName) {
			...
			console.log(event);
			if (tryLockOutput(outputName)) {
				console.log("Scheduler-Triggered Toggle Event Received.")
				const executeTimeout = setTimeout(() => {
					console.warn("Timeout got stuck! Releasing lock forcefully.", outputName);
					releaseOutput(outputName);
